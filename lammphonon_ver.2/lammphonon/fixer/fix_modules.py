...
def fix_thermal_analyzer(ThermalAnalyzer):
    """修复ThermalAnalyzer中的问题"""
    logger.info("修复ThermalAnalyzer中的问题...")
    
    # 获取ThermalAnalyzer类定义所在文件
    file_path = inspect.getfile(ThermalAnalyzer)
    logger.info(f"ThermalAnalyzer定义在: {file_path}")
    ...
    try:
        ...
        if "calculate_heatflux_autocorrelation" in content:
            logger.info("找到calculate_heatflux_autocorrelation方法")
        ...
        if heatflux_data.shape[1] != 3:
            ...
            if heatflux_data.shape[1] > 3:
                ...
                return None"""
                        ...
                        logger.info("已修复ThermalAnalyzer.calculate_heatflux_autocorrelation方法中的形状检查问题")
                        break
            else:
                logger.warning("未找到calculate_heatflux_autocorrelation方法的开始位置")
    except Exception as e:
        logger.error(f"修复ThermalAnalyzer时出错: {str(e)}")

def fix_temporal_analyzer(TemporalAnalyzer):
    """修复TemporalAnalyzer中的问题"""
    logger.info("修复TemporalAnalyzer中的问题...")
    
    # 获取TemporalAnalyzer类定义所在文件
    file_path = inspect.getfile(TemporalAnalyzer)
    logger.info(f"TemporalAnalyzer定义在: {file_path}")
    ...
    try:
        ...
        if "calculate_time_resolved_dos" in content:
            logger.info("找到calculate_time_resolved_dos方法")
        ...
        if window_size < 2:
            ...
            """
                    ...
                    logger.info("已修复TemporalAnalyzer.calculate_time_resolved_dos方法中的窗口尺寸调整问题")
                    found_window_calc = True
                    break
            
            if not found_window_calc:
                logger.warning("未找到calculate_time_resolved_dos方法中的窗口计算部分")
    except Exception as e:
        logger.error(f"修复TemporalAnalyzer时出错: {str(e)}")
        
def fix_equilibration_analyzer(EquilibrationAnalyzer):
    """修复EquilibrationAnalyzer中的问题"""
    logger.info("修复EquilibrationAnalyzer中的问题...")
    
    # 获取EquilibrationAnalyzer类定义所在文件
    file_path = inspect.getfile(EquilibrationAnalyzer)
    logger.info(f"EquilibrationAnalyzer定义在: {file_path}")
    ...
    try:
        ...
        if "calculate_system_equilibration" in content:
            logger.info("找到calculate_system_equilibration方法")
            ...
            if len(energy_values) < 4:
                ...
                return None, None"""
                    ...
                    logger.info("已修复EquilibrationAnalyzer.calculate_system_equilibration方法中的数据点检查问题")
                    found_fitting_code = True
                    break
                    
            if not found_fitting_code:
                logger.warning("未找到calculate_system_equilibration方法中的拟合代码部分")
    except Exception as e:
        logger.error(f"修复EquilibrationAnalyzer时出错: {str(e)}")

def add_missing_methods(EquilibrationAnalyzer):
    """添加缺失的方法"""
    logger.info("添加缺失的方法...")
    
    # 添加analyze_energy_equilibration方法到EquilibrationAnalyzer
    if not hasattr(EquilibrationAnalyzer, 'analyze_energy_equilibration'):
        logger.info("添加EquilibrationAnalyzer.analyze_energy_equilibration方法")
    ...
    def analyze_energy_equilibration(self, energy_values, window_size=10):
        ...
        return equilibration_frame, metric"""
                ...
                logger.info("已添加EquilibrationAnalyzer.analyze_energy_equilibration方法")
            else:
                logger.warning("未找到合适的位置添加新方法")
        except Exception as e:
            logger.error(f"添加EquilibrationAnalyzer.analyze_energy_equilibration方法时出错: {str(e)}")
    else:
        logger.info("EquilibrationAnalyzer.analyze_energy_equilibration方法已存在")

def fix_all_modules(modules):
    """
    修复所有模块
    
    参数:
        modules: 要修复的模块字典，包含各个分析器类
    """
    logger.info("开始应用所有修复补丁...")
    
    # 1. 修复ThermalAnalyzer
    if 'ThermalAnalyzer' in modules:
        fix_thermal_analyzer(modules['ThermalAnalyzer'])
    
    # 2. 修复TemporalAnalyzer
    if 'TemporalAnalyzer' in modules:
        fix_temporal_analyzer(modules['TemporalAnalyzer'])
    
    # 3. 修复EquilibrationAnalyzer
    if 'EquilibrationAnalyzer' in modules:
        fix_equilibration_analyzer(modules['EquilibrationAnalyzer'])
    
    # 4. 添加缺失的方法
    if 'EquilibrationAnalyzer' in modules:
        add_missing_methods(modules['EquilibrationAnalyzer'])
    
    logger.info("所有修复补丁应用完成!") 